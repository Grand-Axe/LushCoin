...
namespace LushCoin
{
	...
	public class ScriptEvaluationContext
	{
		...
		{
			ScriptVerify = LushCoin.ScriptVerify.Standard;
			SigHash = LushCoin.SigHash.Undefined;
...
0xDF,0xE9,0x2F,0x46,0x68,0x1B,0x20,0xA0
		...
		bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker, int sigversion)
		{
			...
			if(!pubkey.Verify(sighash, scriptSig.Signature))
			{
				...
				var R = 4;
				var S = 6 + nLenR;
				var newS = new LushCoin.BouncyCastle.Math.BigInteger(1, vchSig, S, nLenS);
				var newR = new LushCoin.BouncyCastle.Math.BigInteger(1, vchSig, R, nLenR);
		...
		{
			if(SigHash == LushCoin.SigHash.Undefined)
				return true;
