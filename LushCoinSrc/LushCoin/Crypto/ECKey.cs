using LushCoin.BouncyCastle.Asn1.X9;
using LushCoin.BouncyCastle.Crypto.Parameters;
using LushCoin.BouncyCastle.Crypto.Signers;
using LushCoin.BouncyCastle.Math;
using LushCoin.BouncyCastle.Math.EC;
using System;
using LushCoin.BouncyCastle.Math.EC.Custom.Sec;
...
namespace LushCoin.Crypto
{
	internal class ECKey
	{
		...
		static ECKey()
		{
			_Secp256k1 = LushCoin.BouncyCastle.Crypto.EC.CustomNamedCurves.Secp256k1;
		...
		{
			if(isPrivate)
				_Key = new ECPrivateKeyParameters(new LushCoin.BouncyCastle.Math.BigInteger(1, vch), DomainParameter);
			else
			{
		...
		public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, uint256 message, bool compressed)
		{
			...
			var n = curve.N;
			var i = LushCoin.BouncyCastle.Math.BigInteger.ValueOf((long)recId / 2);
			...
			var e = new LushCoin.BouncyCastle.Math.BigInteger(1, message.ToBytes());
			...
			var eInv = LushCoin.BouncyCastle.Math.BigInteger.Zero.Subtract(e).Mod(n);
		...
		private static ECPoint DecompressKey(LushCoin.BouncyCastle.Math.BigInteger xBN, bool yBit)
		{
