...
using LushCoin.BouncyCastle.Crypto;
using LushCoin.Crypto.Internal;
using System;
...
namespace LushCoin.Crypto
{
	...
	internal class Pbkdf2 : Stream
	{
		...
		{
			LushCoin.Crypto.Internal.Check.Null("hmacAlgorithm", hmacAlgorithm);
			LushCoin.Crypto.Internal.Check.Null("salt", salt);
			LushCoin.Crypto.Internal.Check.Length("salt", salt, 0, int.MaxValue - 4);
			LushCoin.Crypto.Internal.Check.Range("iterations", iterations, 1, int.MaxValue);
		...
		{
			LushCoin.Crypto.Internal.Check.Range("count", count, 0, int.MaxValue);
		...
		{
			LushCoin.Crypto.Internal.Check.Range("derivedKeyLength", derivedKeyLength, 0, int.MaxValue);
		...
		{
			LushCoin.Crypto.Internal.Security.Clear(_saltBuffer);
			LushCoin.Crypto.Internal.Security.Clear(_digest);
			LushCoin.Crypto.Internal.Security.Clear(_digestT1);
		...
		{
			LushCoin.Crypto.Internal.Check.Bounds("buffer", buffer, offset, count);
			int bytes = 0;
