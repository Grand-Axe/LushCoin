...
using Builder = System.Func<LushCoin.TransactionBuilder.TransactionBuildingContext, LushCoin.IMoney>;
...
namespace LushCoin
{
	...
	public class TransactionBuilder
	{
		...
		/// <exception cref="LushCoin.NotEnoughFundsException">Not enough funds are available</exception>
		...
		public Transaction SignTransactionInPlace(Transaction transaction, SigHash sigHash)
		{
			...
			{
				var coin = FindSigLushCoinCoin(input);
				if(coin != null)
				{
					Sign(ctx, coin, input);
				}
		...
		public ICoin FindSigLushCoinCoin(IndexedTxIn txIn)
		{
		...
		public int EstimateSize(Transaction tx, bool virtualSize)
		{
			...
			{
				var coin = FindSigLushCoinCoin(txin) ?? FindCoin(txin.PrevOut);
				if(coin == null)
